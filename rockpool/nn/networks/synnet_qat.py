...
from rockpool.nn.modules import LinearTorchQAT, LIFTorchQAT, TimeStepDropout, LIFExodus
...
class SynNetQAT(TorchModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if neuron_model in [LIFTorchQAT, LIFExodus]:
        ...
        for i, (n_hidden, n_tau) in enumerate(
            ...
            lyr_weights =  LinearTorchQAT(shape=(n_channels_in, n_hidden), has_bias=False)
        ...
        lyr_weights = LinearTorchQAT(shape=(n_hidden, n_classes), has_bias=False)
        with torch.no_grad():
    ...
    def forward(self, data: torch.Tensor):
        ...
        # --- collect weights
        linear_modules = [
            m for k, m in self.seq.modules().items()
            if isinstance(m, LinearTorchQAT)
        ]
                
        # compute scales ONCE (or frozen)

        self.global_scale = self.compute_global_scale(linear_modules)
        self.output_scale = self.compute_output_scale(linear_modules)

        # PASS THE SCALES TO THE RESPECTIVE LINEAR MODULES
        for i, module in enumerate(linear_modules):
            ...
            if i == len(linear_modules) - 1:
                ...
                module.qat_scale = self.global_scale
                
    
        neuron_modules = []
        for key, module in self.seq.modules().items():
            if hasattr(module, "threshold"):
                neuron_modules.append(module)
    ...
    def fake_quant_global(self, W_in_list, W_rec_list, scale):
    
        def quantize(W):
            W_q = torch.round(W * scale).clamp(-127, 127) / scale
            return W + self.qat_alpha * (W_q - W).detach()
    
        W_in_q = [quantize(W) for W in W_in_list]
        W_rec_q = [quantize(W) for W in W_rec_list]
    
        return W_in_q, W_rec_q
    
    def fake_quant_output(self, W_out, scale):   
        W_q = torch.round(W_out * scale).clamp(-127, 127) / scale
        W_q = W_out + self.qat_alpha * (W_q - W_out).detach()
        
        return W_q
    
    def compute_global_scale(self, linear_modules):
        input_module = linear_modules[0]
        hidden_modules = linear_modules[1:-1]
    
        # collect all weights
        all_weights = torch.cat(
            [input_module.weight.reshape(-1)] +
            [m.weight.reshape(-1) for m in hidden_modules]
        )
    
        max_w = all_weights.abs().max()
    
        scale = 127.0 / (max_w + 1e-8)
    
        return scale
    
    def compute_output_scale(self, linear_modules):
        output_module = linear_modules[-1]
    
        max_w = output_module.weight.abs().max()
    
        scale_out = 127.0 / (max_w + 1e-8)
